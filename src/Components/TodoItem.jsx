...
const TodoItemSublist = (props) => (
    
    <Wrapper>
        <code>
            [{props.completed ? 'x' : '  '}] <Text onClick={props.onComplete}>{props.title}</Text>
            <Links>
                <Link to="/sublist" onClick={() => props.setCurrentSubListKey(props.data[props.id - 1].id-1)}>See SubList</Link>
            </Links>
    </code>
    </Wrapper>
)
...
//updating the data of the state
export default connect(mapStateToProps, mapDispatchToProp)(TodoItemSublist);
